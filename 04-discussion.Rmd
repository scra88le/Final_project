...
```{r plotAWC, echo=FALSE, message=FALSE, warning=FALSE, fig.asp=0.8, fig.align='center', fig.cap="Available Water Capacity (kPA) across Shetland"}
library(tidyverse)
library(sf)
library(viridis)
...
covars_df %>% 
      ggplot() +
      geom_tile(aes(x=X, y=Y, fill=AWC, width = 1000, height = 1000)) +
      coord_equal() +
      labs(fill="AWC") +
...
The most recent estimates of the Shetland breeding population are from surveys carried out in the late 1990s: 2,300 pairs of Curlew, 1,740 pairs of Lapwing, 3,350 pairs of Oystercatcher, 1,170 pairs of Redshank and 3,450 pairs of Snipe (Harvey et al. 2004)
