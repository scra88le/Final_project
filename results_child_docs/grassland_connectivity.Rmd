...
improved_patches <- res_r %>% 
  lsm_p_area() %>% 
  filter(class==1 & value >= 2) %>%
  pull(id)

# Get all points that are in improved patches >= 1Ha
patches_sf <- get_patches(res_r, class = 1)[[1]] %>% 
  rasterToPoints() %>% 
  as.data.frame() %>% 
...
improved_polys_sf <- patches_sf %>% 
  ...
  mutate(chull_poly = map_dfr(
    .x=data,
    ~{st_union(.x) %>% st_convex_hull() %>% st_as_sf(crs=27700)}
  )) %>%
  dplyr::select(-data)
...
First generate an adjaceny matrix between breeding sites and improved grassland sites.
