...
```{r loadSentinelImg, echo=FALSE, message=FALSE, warning=FALSE,fig.align='center', fig.asp=1.2, fig.cap='Clipped Sentinel 2 RGB composite of Shetland'}
# 2020 July Product
sent = list.files("~/Documents/GitHub/shetlandwaders/data/shetland_sentinel/20m/", 
                  pattern = ".tiff", 
                  full.names = TRUE)
# Create raster stack
sent = stack(sent)
...
```{r satBands, echo=FALSE, message=FALSE, warning=FALSE, fig.cap='Sentinel 2 spectral sensors'}
# Sentinel 2 Bands, central wavelength(micrometre), all at 10m resolution
sensors <- data.frame(
              band = c('1','2','3','4','5','6','7','8','8A','11','12'),
              name = c('Aerosol','Blue','Green','Red','Veg Red 1','Veg Red 2','Veg Red 3','NIR','Veg Red 4','SWIR 1','SWIR 2'),
              wave = c(0.443,0.490,0.560,0.655,0.705,0.865, 0.740,0.783,0.842,1.610,2.190)
              )

# knit table
sensors %>% 
  knitr::kable(
    col.names = c("Band ID","Name","Wavelength(micrometer)"),
    booktabs = TRUE
  )
```
...
training_classes <- st_read("~/Documents/GitHub/shetlandwaders/data/training_data_classification/Training_samples.shp", crs=27700)
...
```{r searchGrid, echo=FALSE, message=FALSE, warning=FALSE, fig.cap='Search parameters used for model fitting of support vector machine'}
...
```{r tune_model, , message=FALSE, warning=FALSE, include=FALSE}
...
```{r modelResults,echo=FALSE, message=FALSE, warning=FALSE, fig.cap='SVM model parameters from best fitting models, as measured by classification accuracy of 1-fold validation over training dataset' }
...
```{r evalPerf, echo=FALSE, message=FALSE, warning=FALSE, fig.cap='Classifier accuracy against test data set'}
...
```{r classify_raster, message=FALSE, warning=FALSE, include=FALSE}
